...
func depositToDisputeFund(
    fund: felt, caller_address: felt, token_id: Uint256, amount_to_deposit: Uint256
) {
}
...
func balanceOf{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    account: felt, id: Uint256, fund: felt
) -> (balance: Uint256) {
...
@view
func getSecurityFundBalance{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    fund: felt
) -> (balance: Uint256) {
...
func deposit{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    fund: felt, token_id: Uint256, amount_to_deposit: Uint256
) {
    alloc_locals;
...
func withdraw{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    fund: felt, token_id: Uint256, amount: Uint256
) {
    alloc_locals;
...
func managerDeposit{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    fund: felt, token_id: Uint256, amount: Uint256
) {
...
@external
func managerWithdraw{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    fund: felt, token_id: Uint256, amount: Uint256
) {
    alloc_locals;
    let (asset_man) = IFuccount.manager(fund);
...
func resultDispute{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    bool: felt, fund: felt
) {
    alloc_locals;
    ...
    if (bool == TRUE) {
        ...
        ) = ownerShares(fund, asset_manager);
        IFuccount.burnBatch(
            fund, contract_address, assetId_len, assetId, assetAmount_len, assetAmount
        );
        ...
        let (
            assetId_len: felt, assetId: Uint256*, assetAmount_len: felt, assetAmount: Uint256*
        ) = IFuccount.ownerShares(fund, contract_address);
        let (
            assetIdAM_len: felt,
            assetIdAM: Uint256*,
            assetAmountAM_len: felt,
            assetAmountAM: Uint256*,
        ) = ownerShares(fund, asset_manager);
        ...
        ) = get_all_shares_from_dispute_fund(
            assetId_len,
            assetId,
            assetAmount_len,
            assetAmount,
        ...
        IFuccount.burnBatch(
            fund, contract_address, assetIdWAM_len, assetIdWAM, assetIdWAM_len, assetAmountWAM
        );
...
func get_all_shares_from_dispute_fund{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(
    assetIdAll_len: felt,
    assetIdAll: Uint256*,
    assetAmountAll_len: felt,
    assetAmountAll: Uint256*,
    assetIdAM_len: felt,
    assetIdAM: Uint256*,
    assetAmountAM_len: felt,
    assetAmountAM: Uint256*,
    assetIdWAM_len: felt,
    assetIdWAM: Uint256*,
    assetAmountWAM_len: felt,
    assetAmountWAM: Uint256*,
) -> (
    assetIdWAM_len: felt, assetIdWAM: Uint256*, assetAmountWAM_len: felt, assetAmountWAM: Uint256*
) {
    alloc_locals;
    if (assetIdAll_len == 0) {
        ...
        if (assetIdAM_len == 0) {
            ...
            return get_all_shares_from_dispute_fund(
                assetIdAll_len=assetIdAll_len - 1,
                assetIdAll=assetIdAll + 2,
                assetAmountAll_len=assetAmountAll_len - 1,
                assetAmountAll=assetAmountAll + 2,
                assetIdAM_len=assetIdAM_len,
                assetIdAM=assetIdAM,
                assetAmountAM_len=assetAmountAM_len,
                assetAmountAM=assetAmountAM,
                assetIdWAM_len=assetIdWAM_len + 1,
                assetIdWAM=assetIdWAM + 2,
                assetAmountWAM_len=assetAmountWAM_len + 1,
                assetAmountWAM=assetAmountWAM + 2,
            );
        } else {
            ...
            if (condition1 * condition2 == 1) {
                return get_all_shares_from_dispute_fund(
                    ...
                    assetAmountAM_len=assetAmountAM_len,
                    assetAmountAM=assetAmountAM,
                    assetIdWAM_len=assetIdWAM_len,
                    assetIdWAM=assetIdWAM,
                    assetAmountWAM_len=assetAmountWAM_len,
                    assetAmountWAM=assetAmountWAM,
                );
            }
        }
    }
    return get_all_shares_from_dispute_fund(
        assetIdAll_len=assetIdAll_len,
        assetIdAll=assetIdAll,
        assetAmountAll_len=assetAmountAll_len,
        assetAmountAll=assetAmountAll,
        assetIdAM_len=assetIdAM_len - 1,
        assetIdAM=assetIdAM + 2,
        assetAmountAM_len=assetAmountAll_len - 1,
        assetAmountAM=assetAmountAM + 2,
        assetIdWAM_len=assetIdWAM_len,
        assetIdWAM=assetIdWAM,
        assetAmountWAM_len=assetAmountWAM_len,
        assetAmountWAM=assetAmountWAM,
...
func ownerShares{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    fund: felt, account: felt
) -> (assetId_len: felt, assetId: Uint256*, assetAmount_len: felt, assetAmount: Uint256*) {
    alloc_locals;
    let (totalId_: Uint256) = IFuccount.totalId(fund);
    let (local assetId: Uint256*) = alloc();
    let (local assetAmount: Uint256*) = alloc();
    let (tabSize_: felt) = completeMultiAssetTab(
        totalId_, 0, assetId, 0, assetAmount, account, fund
    );
    return (tabSize_, assetId, tabSize_, assetAmount);
}

func completeMultiAssetTab{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    totalId: Uint256,
    assetId_len: felt,
    assetId: Uint256*,
