...
@event
func transfer_single(operator: felt, from_: felt, to: felt, id: Uint256, value: Uint256) {
}
...
namespace FuccountLib {
    ...
    func initializer{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        _public_key: felt, _vault_factory: felt
    ) {
    ...
    func get_share_price_purchased{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        tokenId: Uint256
    ) -> (res: Uint256) {
    ...
    func get_shares_total_supply{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
        ) -> (totalSupply: Uint256) {
        let (totalSupply: Uint256) = shares_total_supply.read();
        return (totalSupply,);
    }

    func get_total_id{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}() -> (
        res: Uint256
    ) {
    ...
    func owner_shares{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        account: felt
        ...
        let (tabSize_: felt) = _complete_multi_share_tab(
            total_id_, 0, assetId, 0, assetAmount, account
        );
    ...
    func get_vault_factory{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (
        res: felt
    ) {
    ...
    func get_manager_account{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (
        res: felt
    ) {
    ...
    func get_denomination_asset{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ) -> (res: felt) {
    ...
    func get_share_balance{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        _share: felt, _id: Uint256
    ) -> (shareBalance_: Uint256) {
        let (account_: felt) = get_contract_address();
        let (shareBalance_: Uint256) = IFuccount.balanceOf(account_, _share, _id);
        return (shareBalance_,);
    }

    func get_not_nul_assets{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (
        notNulAssets_len: felt, notNulAssets: AssetInfo*
    ) {
        alloc_locals;
        let (IM_: felt) = _get_integration_manager();
        let (
            availableAssets_len: felt, availableAssets: felt*
        ) = IIntegrationManager.availableAssets(IM_);
        let (local notNulAssets: AssetInfo*) = alloc();
        let (notNulAssets_len: felt) = _complete_non_nul_asset_tab(
            availableAssets_len, availableAssets, 0, notNulAssets
        );
        return (notNulAssets_len, notNulAssets);
    }

    func get_not_nul_shares{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (
        ...
        let (
            availableAShares_len: felt, availableShares: felt*
        ) = IIntegrationManager.availableShares(IM_);
        let (local notNulShares: ShareInfo*) = alloc();
        let (notNulShares_len: felt) = _complete_non_nul_shares_tab(
            availableAShares_len, availableShares, 0, notNulShares, selfAddress, denomination_asset_
        );
        return (notNulShares_len, notNulShares);
    }

    func get_not_nul_positions{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ) -> (notNulPositions_len: felt, notNulPositition: felt*) {
        alloc_locals;
        let (IM_: felt) = _get_integration_manager();
        let (
            availableExternalPositions_len: felt, availableExternalPositions: felt*
        ) = IIntegrationManager.availableExternalPositions(IM_);
        let (local notNulExternalPositions: PositionInfo*) = alloc();
        let (notNulExternalPositions_len: felt) = _complete_non_nul_position_tab(
            availableExternalPositions_len, availableExternalPositions, 0, notNulExternalPositions
        );
    ...
    func get_asset_value{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (value_: Uint256) = IValueInterpretor.calculAssetValue(
            valueInterpretor_, _asset, _amount, _denomination_asset
        );
        return (res=value_);
    }
    ...
    func getShareValue{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (valueInterpretor_: felt) = _get_value_interpretor();
        let (value_: Uint256) = IValueInterpretor.calculAssetValue(
            valueInterpretor_, _share, Uint256(_amount.low, _id.low), _denomination_asset
        );
        return (res=value_);
    }
    ...
    func is_free_reedem{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        amount: Uint256
    ) -> (is_free_reedem: felt) {
        alloc_locals;
        ...
        let (
            allowedAssetToReedem_len: felt, allowedAssetToReedem: felt*
        ) = IPolicyManager.allowedAssetsToReedem(policyManager_, contractAddress_);
        if (allowedAssetToReedem_len == 0) {
            return (1,);
        } else {
            let (denomination_asset_) = denomination_asset.read();
            let (valueInDeno_: Uint256) = _sum_value_in_deno(
                allowedAssetToReedem_len, allowedAssetToReedem, denomination_asset_
            );
            let (sharePrice_) = get_share_price();
            let (shareValueInDenoTemp_: Uint256) = SafeUint256.mul(sharePrice_, amount);
            let (shareValueInDeno_: Uint256, _) = SafeUint256.div_rem(
                shareValueInDenoTemp_, Uint256(POW18, 0)
            );
    ...
    func is_available_reedem{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        amount: Uint256
    ) -> (is_available_reedem: felt) {
        alloc_locals;
    ...
    func share_to_deno{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        _reedem_tab(
            1,
            assetAmounts,
            performancePermillion_,
            durationPermillion_,
            fund_,
            0,
            assetCallerAmount,
            assetManagerAmount,
            assetStackingVaultAmount,
            assetDaoTreasuryAmount,
        );
    ...
    func preview_reedem{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (remainingValue_: Uint256, len: felt) = _calc_amount_of_each_asset(
            sharesValue_, assets_len, assets, 0, assetAmounts, denomination_asset_
        );
        let (isRemaingValueNul_: felt) = uint256_eq(remainingValue_, Uint256(0, 0));
        if (isRemaingValueNul_ == 0) {
            let (remainingValue2_: Uint256, len2: felt) = _calc_amount_of_each_share(
                remainingValue_, shares_len, shares, 0, shareAmounts, denomination_asset_
            );
            ...
            _reedem_tab(
                ...
                shareCallerAmount,
                shareManagerAmount,
                shareStackingVaultAmount,
                shareDaoTreasuryAmount,
            );
            return (
                len,
                assetCallerAmount,
    ...
    func mint{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        to: felt, sharesAmount: Uint256, _sharePricePurchased: Uint256
    ) {
    ...
    func _assert_allowed_asset_to_reedem{
        ...
        let (isAllowedAssetToReedem_) = IPolicyManager.isAllowedAssetToReedem(
            policyManager, contractAddress, asset[0]
        );
        with_attr error_message(
                "_assert_allowed_asset_to_reedem:  Only allowed assets can be reedem") {
            assert isAllowedAssetToReedem_ = 1;
        }
        return _assert_allowed_asset_to_reedem(
            asset_len - 1, asset + 1, policyManager, contractAddress
        );
    }
    ...
    func get_fund_level{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (
        res: felt
    ) {
    ...
    func preview_deposit{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (fee, fee_assset_manager, fee_treasury, fee_stacking_vault) = _get_fee(
            fund_, FeeConfig.ENTRANCE_FEE, _amount
        );
        let (sharePrice_) = get_share_price();
    ...
    func revoke_result{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        isReportAccepted: felt
    ) {
        alloc_locals;
        let (vaultFactory_: felt) = vault_factory.read();
        let (stackingDispute_: felt) = IVaultFactory.getStackingDispute();
    ...
    func deposit{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (
            shareAmount_: Uint256,
            fundAmount_: Uint256,
            managerAmount_: Uint256,
            treasuryAmount_: Uint256,
            stackingVaultAmount_: Uint256,
        ) = preview_deposit(_amount);
    ...
    func reedem{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (
            ...
            len2: felt,
            shareDaoTreasuryAmount: Uint256*,
        ) = preview_reedem(id, amount, assets_len, assets, shares_len, shares);
    ...
    func set_fund_level{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        _fund_level: felt
    ) {
        only_vault_factory();
    ...
    func _sum_value_in_deno{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        asset_len: felt, asset: felt*, denomination_asset: felt
    ) -> (valueInDeno: Uint256) {
        alloc_locals;
        if (asset_len == 0) {
            return (Uint256(0, 0),);
        ...
        let (AssetvalueInDeno_: Uint256) = get_asset_value(
            asset[0], fundAssetBalance, denomination_asset
        );
        let (valueOfRest_: Uint256) = _sum_value_in_deno(
            asset_len - 1, asset + 1, denomination_asset
        );
    ...
    func _check_call{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (isIntegrationAvailable_) = IIntegrationManager.isAvailableIntegration(
            integrationManager_, _contract, _selector
        );
        ...
        let (integrationLevel_) = IIntegrationManager.integrationRequiredFundLevel(
            integrationManager_, _contract, _selector
        );
        ...
        let (preLogicContract: felt) = IIntegrationManager.prelogicContract(
            integrationManager_, _contract, _selector
        );
    ...
    func _get_fee{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        _vault: felt, key: felt, amount: Uint256
    ) -> (
        fee: Uint256, fee_asset_manager: Uint256, fee_treasury: Uint256, fee_stacking_vault: Uint256
    ) {
        alloc_locals;
        ...
        return (
            fee=fee,
            fee_asset_manager=fee_asset_manager,
            fee_treasury=fee_treasury,
            fee_stacking_vault=fee_stacking_vault,
        );
    }

    func _calc_amount_of_each_asset{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(
        totalValue: Uint256,
        asset_len: felt,
        asset: felt*,
        assetAmount_len: felt,
        assetAmount: Uint256*,
        denomination_asset: felt,
    ) -> (remainingValue: Uint256, len: felt) {
        alloc_locals;
        if (asset_len == 0) {
        ...
        if (isAssetFundBalanceNul_ == 1) {
            return _calc_amount_of_each_asset(
                totalValue,
                asset_len - 1,
                asset + 1,
                assetAmount_len,
            ...
            let (assetvalueInDeno_: Uint256) = get_asset_value(
                asset[0], assetFundbalance_, denomination_asset
            );
            let (isAssetFundBalanceEnought: felt) = uint256_le(totalValue, assetvalueInDeno_);
            if (isAssetFundBalanceEnought == 1) {
                let (requiredAmount: Uint256) = get_asset_value(
                    denomination_asset, totalValue, asset[0]
                );
    ...
    func _calc_amount_of_each_share{
        syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
    }(
        totalValue: Uint256,
        share_len: felt,
        share: ShareWithdraw*,
        shareAmount_len: felt,
        shareAmount: Uint256*,
        denomination_asset: felt,
    ) -> (remainingValue: Uint256, len: felt) {
        alloc_locals;
        ...
        if (isShareFundBalanceNul_ == 1) {
            return _calc_amount_of_each_share(
                totalValue,
                share_len - 1,
                share + ShareWithdraw.SIZE,
                shareAmount_len,
                shareAmount,
                denomination_asset,
            );
        } else {
            let (sharevalueInDeno_: Uint256) = getShareValue(
                share[0].address, share[0].id, shareFundbalance_, denomination_asset
            );
            let (isShareFundBalanceEnought: felt) = uint256_le(totalValue, sharevalueInDeno_);
            if (isShareFundBalanceEnought == 1) {
                let (oneSharevalueInDeno_: Uint256) = getShareValue(
                    share[0].address, share[0].id, Uint256(POW18, 0), denomination_asset
                );
    ...
    func _transfer_each_asset{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        assets_len: felt,
        assets: felt*,
        callerAmount: Uint256*,
        managerAmount: Uint256*,
        stackingVaultAmount: Uint256*,
        daoTreasuryAmount: Uint256*,
    ) {
        alloc_locals;
        if (assets_len == 0) {
            return ();
        ...
        return _transfer_each_asset(
            ...
            assets_len - 1,
            assets + 1,
            callerAmount + Uint256.SIZE,
            managerAmount + Uint256.SIZE,
            stackingVaultAmount + Uint256.SIZE,
            daoTreasuryAmount + Uint256.SIZE,
        );
    }
    ...
    func __calcul_gav_position{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (gavOfRest) = __calcul_gav_position(
            externalPositions_len=externalPositions_len - 1, externalPositions=externalPositions
        );
        let (gav, _) = uint256_add(asset_value, gavOfRest);
        return (gav=gav);
    }
    ...
    func _assert_allowed_depositor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        if (isPublic_ == 1) {
            ...
            let (isAllowedDepositor_: felt) = IPolicyManager.isAllowedDepositor(
                policyManager_, fund_, caller
            );
    ...
    func _calcul_tab_100{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        _percents_len: felt, _percents: felt*
    ) -> (res: felt) {
        alloc_locals;
        if (_percents_len == 0) {
            return (0,);
    ...
    func _assert_enought_guarantee{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (securityFundBalance_) = IStackingDispute.getSecurityFundBalance(
            stackingDispute_, contractAddress_
        );
        ...
        with_attr error_message(
                "_assert_enought_guarantee: Asser manager need to provide more guarantee ") {
            assert_not_zero(isEnoughtGuarantee_);
        }
        return ();
    }

    func _withdraw_asset_to{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        _asset: felt, _target: felt, _amount: Uint256
    ) {
        let (_success) = IERC20.transfer(
            contract_address=_asset, recipient=_target, amount=_amount
        );
    ...
    func _withdraw_share_to{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        _fund: felt, _target: felt, _share: felt, _id: Uint256, _amount: Uint256
    ) {
    ...
    func _reedem_tab{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        len: felt,
        amount: Uint256*,
        performancePermillion: Uint256,
        durationPermillion: felt,
        fund: felt,
        tabLen: felt,
        callerAmount: Uint256*,
        managerAmount: Uint256*,
        ...
        let (fee0, feeAssetManager0, feeDaoTreasury0, feeStackingVault0) = _get_fee(
            fund, FeeConfig.PERFORMANCE_FEE, performanceAmount_
        );

        let (remainingAmount0_: Uint256) = uint256_sub(amount_, fee0);

        // MANAGEMENT FEES
        let (millionTimeDuration_: Uint256) = uint256_mul_low(
            amount_, Uint256(durationPermillion, 0)
        );
        let (managementAmount_: Uint256) = uint256_div(millionTimeDuration_, Uint256(PRECISION, 0));
        let (fee1, feeAssetManager1, feeDaoTreasury1, feeStackingVault1) = _get_fee(
            fund, FeeConfig.MANAGEMENT_FEE, managementAmount_
        );

        let (remainingAmount1_: Uint256) = uint256_sub(remainingAmount0_, fee1);
        let (cumulativeFeeAssetManager1: Uint256, _) = uint256_add(
            feeAssetManager0, feeAssetManager1
        );
        let (cumulativeFeeStackingVault1: Uint256, _) = uint256_add(
            feeStackingVault0, feeStackingVault1
        );
        let (cumulativeFeeDaoTreasury1: Uint256, _) = uint256_add(feeDaoTreasury0, feeDaoTreasury1);

        // EXIT FEES
        let (fee2, feeAssetManager2, feeDaoTreasury2, feeStackingVault2) = _get_fee(
            fund, FeeConfig.EXIT_FEE, amount_
        );
        let (remainingAmount2_: Uint256) = uint256_sub(remainingAmount1_, fee2);
        let (cumulativeFeeAssetManager2: Uint256, _) = uint256_add(
            cumulativeFeeAssetManager1, feeAssetManager2
        );
        let (cumulativeFeeStackingVault2: Uint256, _) = uint256_add(
            cumulativeFeeStackingVault1, feeStackingVault2
        );
        let (cumulativeFeeDaoTreasury2: Uint256, _) = uint256_add(
            cumulativeFeeDaoTreasury1, feeDaoTreasury2
        );
    ...
    func _complete_non_nul_asset_tab{
        ...
        availableAssets_len: felt,
        availableAssets: felt*,
        notNulAssets_len: felt,
        notNulAssets: AssetInfo*,
    ) -> (notNulAssets_len: felt) {
        alloc_locals;
        ...
        if (isZero_ == 0) {
            ...
            let (denomination_asset_: felt) = denomination_asset.read();
            let (assetValue: Uint256) = get_asset_value(
                assetIndex_, assetBalance_, denomination_asset_
            );
    ...
    func _complete_non_nul_shares_tab{
        ...
        availableAShares_len: felt,
        availableShares: felt*,
        notNulShares_len: felt,
        notNulShares: ShareInfo*,
        selfAddress: felt,
        denomination_asset: felt,
    ) -> (notNulShares_len: felt) {
        alloc_locals;
        ...
        let (
            assetId_len: felt, assetId: Uint256*, assetAmount_len: felt, assetAmount: Uint256*
        ) = IFuccount.ownerShares(fundAddress_, selfAddress);
        if (assetId_len == 0) {
        ...
        let (newTabLen) = _complete_share_info(
            assetId_len,
            selfAddress,
            assetId,
            assetAmount,
            0,
            notNulShares + notNulShares_len,
            denomination_asset,
        );
        return _complete_non_nul_shares_tab(
            availableAShares_len=availableAShares_len - 1,
    ...
    func _complete_share_info{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        len: felt,
        fundAddress: felt,
        assetId: Uint256*,
        assetAmount: Uint256*,
        ...
        let (valueInDeno_) = getShareValue(
            fundAddress,
            assetId[shareInfo_len - 1],
            assetAmount[shareInfo_len - 1],
            denomination_asset_,
        );
    ...
    func _complete_non_nul_position_tab{
        ...
        availableExternalPositions_len: felt,
        availableExternalPositions: felt*,
        notNulExternalPositions_len: felt,
        notNulExternalPositions: PositionInfo*,
    ) -> (notNulExternalPositions_len: felt) {
        alloc_locals;
        ...
        let (value_: Uint256) = IValueInterpretor.calculAssetValue(
            VI_, externalPositionIndex_, Uint256(contractAddress_, 0), denomination_asset_
        );
    ...
    func _complete_multi_share_tab{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        total_id: Uint256,
        assetId_len: felt,
        assetId: Uint256*,
        assetAmount_len: felt,
    ...
    func reduce_supply_batch{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (currentTotalSupply_) = shares_total_supply.read();
        let (newTotalSupply_) = uint256_sub(
            currentTotalSupply_, amounts[amounts_len * Uint256.SIZE - Uint256.SIZE]
        );
        shares_total_supply.write(newTotalSupply_);
        return reduce_supply_batch(amounts_len=amounts_len - 1, amounts=amounts);
    }
    ...
    func _safe_transfer_from{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        transfer_single.emit(operator, from_, to, id, amount);
        return ();
    }
    ...
    func _safe_batch_transfer_from{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        // Emit events and check
        let (operator) = get_caller_address();
        transfer_batch.emit(operator, from_, to, ids_len, ids, amounts_len, amounts);
        return ();
    }
    ...
    func _mint{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        // Emit events and check
        let (operator) = get_caller_address();
        transfer_single.emit(operator=operator, from_=0, to=to, id=id, value=amount);
        return ();
    }
