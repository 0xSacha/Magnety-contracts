...
@view
func calcAssetValueBmToDeno{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    _baseAsset: felt, _amount: Uint256, _denominationAsset: felt
) -> (res: Uint256) {
    alloc_locals;
    ...
    let (
        denominationAssetRateFelt_: felt, decimalsDenominationAssetRate: felt, _, _
    ) = IEmpiricOracle.get_value(empiricOracle_, denominationAssetKey_, 0);
    let (baseAssetRateFelt_: felt, decimalsBaseAssetRate: felt, _, _) = IEmpiricOracle.get_value(
        empiricOracle_, baseAssetAggregatorKey_, 0
    );
    ...
    let (decimalsDenominationAssetRateUint256_: Uint256) = felt_to_uint256(
        decimalsDenominationAssetRate
    );
    let (decimalsBaseAssetRateUint256_: Uint256) = felt_to_uint256(decimalsBaseAssetRate);
    let (denoLowerThanBase_) = uint256_lt(
        decimalsDenominationAssetRateUint256_, decimalsBaseAssetRateUint256_
    );
    let (baseLowerThanDeno_) = uint256_lt(
        decimalsBaseAssetRateUint256_, decimalsDenominationAssetRateUint256_
    );
    ...
    let (decimalsDenominationAssetPow: Uint256) = uint256_pow(
        Uint256(10, 0), denominationAssetDecimals
    );
...
@external
func addPrimitive{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    _asset: felt, _key: felt
) {
    onlyOwner();
